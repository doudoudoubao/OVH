...
func GetNewPlanWatch(state *app.State, mon *monitor.Monitor) gin.HandlerFunc {
	return func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{
			...
			// siteCount 有账户的站点数 = 每轮要各拉一份 12MB 目录的份数。
			// 不能让前端从 currencyHint 的键数去推:那份提示是**只读缓存**的
			// (见 subsidiaryCurrencies 的说明),缓存冷时是空的,
			// 带宽估算会凭空变成 0。
			"siteCount":       len(accountSubsidiaries(state)),
...
// accountSubsidiaries 账户表里出现过的全部子公司(去重)。
func accountSubsidiaries(state *app.State) map[string]struct{} {
...
func subsidiaryCurrencies(state *app.State) map[string]string {
	...
	for s := range accountSubsidiaries(state) {
		// **只读缓存,拿不到就算了**。这是个同步 HTTP handler:走会触发拉取的路径的话,
		// 缓存冷的时候打开监控页就会卡在这里等 N × 12MB 下载完。
		// 币种只是个填表提示,值不上让整个页面等。
		if cur, ok := catalog.CachedCurrency(s); ok {
			out[s] = cur
		}
	}
	return out
