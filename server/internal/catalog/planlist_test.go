...
// CachedCurrency 绝不能触发拉取:它给的是同步 HTTP handler 用的填表提示,
// 一触发就是让一个页面请求等一份 12MB 的目录下载完。
func TestCachedCurrencyNeverFetches(t *testing.T) {
	calls := stubCatalogFetch(t, twoPlanCatalog(), nil)

	// 缓存冷:拿不到就该直接说拿不到,而不是去拉
	if cur, ok := CachedCurrency("IE"); ok {
		t.Errorf("缓存冷时应当返回 false，实际给了 %q", cur)
	}
	if n := atomic.LoadInt32(calls); n != 0 {
		t.Fatalf("CachedCurrency 绝不能触发拉取，实际拉了 %d 次", n)
	}
	...
	cur, ok := CachedCurrency("ie") // 顺带验大小写归一
	if !ok || cur != "EUR" {
		t.Errorf("预热后应当拿到 EUR，实际 %q(ok=%v)", cur, ok)
	}
	if n := atomic.LoadInt32(calls); n != before {
		t.Errorf("命中缓存也不该额外拉取，实际多拉了 %d 次", n-before)
	}
}
