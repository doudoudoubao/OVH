...
//   - 超上限:本来就该拦。
//
// 全部判 Fatal:同一个 planCode + 同一套配置,重试一万次价格还是这个价。
// 不判 Fatal 的话任务会按 retryInterval 永远刷下去,每轮再写一条一模一样的失败 history。
//
// # 抢购主链路的开销
//
...
func enforceMonthlyCap(state *app.State, item *types.QueueItem, addons []string) (Outcome, bool) {
	...
	if err != nil {
		return block(fmt.Sprintf(
			"设了月费上限 %s,但算不出这套配置的价格(%s)——价格未知时一律不下单。",
	...
	if price.Partial {
		return block(fmt.Sprintf(
			"设了月费上限 %s,但这套配置里有 addon(%v)不在目录里，"+
				"算出来的 %.2f 是**偏低**的数字，拿它跟上限比等于没有上限，因此不下单。",
