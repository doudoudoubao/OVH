...
> **`npm run build` 的两步顺序是 `vite build && tsc -b`,别对调。**
...
监控页最上面有一块「新机型发现」。打开后会按设定的间隔（默认 60 分钟，最短 15 分钟）
重拉一份 OVH 的公开 eco 目录，和上一次的机型集合比对，冒出没见过的 `planCode` 就推通知。

**第一轮只建立基线、不告警** —— 否则开机会把目录里几百个机型全当成「新上架」。

可选再打开「自动下单」：命中的新机型会被自动建一条「有货就买」的监控订阅。
这条路径有两道价格闸：

1. 发现时用**目录基础月费**预筛（不含内存/硬盘加价，所以只是预筛）；
2. 真正算数的一道在 `PurchaseServer` 结账之前 —— 那时候 addon 才刚从
   「当时有货的那条 FQN」推出来，价格才是真的。超限、币种对不上、
   报价不完整、算不出价，四种情况一律**不下单**。

> **它永远不会自动付款。** 这条路径没有 autoPay 开关（不是默认关，是压根没有）。
> 订单停在 `notPaid`，逾期自动作废，付不付由你自己决定。

本程序**不做汇率换算**：上限的币种必须和账户所在站点的计价币种一致，对不上就不下单。
...
更新源默认指向上游 `gokele/ovh`。**跑自己的 fork 却不改它,点一次「在线更新」
就会把你的版本覆盖成上游的**,自己的改动全没 —— 而且不报错,看起来就是一次正常更新。

`.env` 里加一行指到自己的仓库即可,**运行时读取,不用重新编译**:

```bash
OVH_UPDATE_API=https://api.github.com/repos/<你的用户名>/<仓库名>/releases/latest
```

检查更新和真正下载用的是同一个地址,不会出现"显示有新版、装上的却是另一个"。
自己发版时 Release 里要带:

| 附件 | 说明 |
|---|---|
| `ovh-server-<系统>-<架构>`(Windows 多个 `.exe`) | 与 `updater.AssetName()` 的命名一致 |
| `checksums.txt` | 各产物的 sha256。**没有它会直接拒绝更新**,不给"要不要冒险"的选项 |

版本号只比前三段,而且不能用四段或后缀 —— `parseSemver` 遇到 `-` / `+` 会截断,
四段会让第三段 `Atoi` 失败而归零:
...
| `0.1.16` | `[0 1 16]` | ✅ |
| `0.2.0` | `[0 2 0]` | ✅ |
| `0.1.15-1` | `[0 1 15]` | ❌ 不算更新 |
| `0.1.15.1` | `[0 1 0]` | ❌ **比 0.1.15 还旧** |
