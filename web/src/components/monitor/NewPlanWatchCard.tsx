...
/** 每轮 siteCount 份 12MB 目录，按间隔折算成每天多少流量。 */
function dailyMB(siteCount: number, intervalMinutes: number): string {
  const perDay = (siteCount * 12 * 24 * 60) / intervalMinutes;
  return perDay >= 1024 ? `${(perDay / 1024).toFixed(1)}GB` : `${Math.round(perDay)}MB`;
}
...
export function NewPlanWatchCard() {
  ...
  // 带宽估算用后端给的站点数。**不要从 currencyHint 的键数去推** ——
  // 那份币种提示是只读缓存的，缓存冷时为空，估算会凭空变成 0。
  const siteCount = q.data!.siteCount ?? 0;
          ...
          <div className="space-y-4 pt-1 border-t">
            ...
            <div className="grid gap-1.5">
              ...
              <p className="text-[11px] text-muted-foreground">
                每轮会按**站点**各重拉一份约 12MB 的公开目录（不占账户 API 配额）。
                {siteCount > 0 && cfg.intervalMinutes > 0
                  ? `你现在有 ${siteCount} 个站点，按这个间隔约 ${dailyMB(siteCount, cfg.intervalMinutes)}/天。`
                  : ""}
